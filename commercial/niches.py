...
def load_niches() -> tuple[dict[str, Any], ...]:
    ...
    if registry["niche_count"] != len(niches):
        raise NicheError(f"niche_count declara {registry['niche_count']} mas há {len(niches)} nichos")
    ids = [n["niche_id"] for n in niches]
    if len(set(ids)) != len(ids):
        raise NicheError("niche_id duplicado")

    valid_areas = {area["legal_area_id"] for area in json.loads(TAXONOMY_PATH.read_text(encoding="utf-8"))["areas"]}
    for niche in niches:
    ...
    for niche in load_niches():
        for area_id in niche["legal_area_ids"]:
            ...
            if area_id in mapping and mapping[area_id] != niche["niche_id"]:
                raise NicheError(f"área {area_id!r} reivindicada por {mapping[area_id]!r} e {niche['niche_id']!r}")
