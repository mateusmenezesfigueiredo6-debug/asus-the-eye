...
def backtest(y: np.ndarray, nome: str) -> dict:
    ...
    return {"avaliado": True, "n": len(r), "mae": round(mae(r, p), 2),
...
def avaliar_nicho(nid: str, pontos: dict, meses: list[str], h: int) -> dict:
    ...
    if observados < MIN_PONTOS:
        return {"niche_id": nid, "status": "serie_insuficiente",
    ...
    regime = {
        "holt": ("tendencia", "ha sinal de tendencia: o modelo com inclinacao "
                 "vence a media simples"),
        "ingenuo": ("passeio_aleatorio", "serie sem memoria util: o ultimo valor "
                    "e o melhor previsor, prever adiante nao agrega"),
        "media3": ("estavel", "serie estavel em torno da media: nao ha tendencia "
                   "a extrair"),
    }[campeao]
...
def main(h: int = HORIZONTE_PADRAO) -> None:
    ...
    print(f"   {len(avaliados)}/{len(avaliacoes)} nichos avaliados · "
          f"{len(com_ganho)} com ganho preditivo real")
    for a in avaliados[:10]:
        print(f"   {a['niche_id']:<20} {a['regime']:<16} campeao={a['modelo_campeao']:<8} "
