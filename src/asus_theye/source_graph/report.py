...
def coverage_report(coverage: dict[str, Any], tracks: dict[str, Any]) -> str:
    ...
    for track, data in tracks.items():
        ...
        lines.append(f"| {track} | {data['qualified']} | {data['target']} | {data['coverage_pct']}% | {reasons} |")
    ...
    lines += [
        ...
        "## Por nicho",
        "",
        f"{covered_niches} de {len(coverage['by_niche'])} nichos têm ao menos uma fonte qualificada.",
        "",
    ]
    return "\n".join(lines)
...
def gaps_report(coverage: dict[str, Any]) -> str:
    ...
    for gap in coverage["gaps"]:
        ...
        lines.append(f"| {reason} | {gap['description']} | {gap.get('what_would_unblock', '—')} |")
    lines.append("")
    return "\n".join(lines)
