...
class PoliteFetcher:
    ...
    def __post_init__(self) -> None:
        if not self.license_id:
            raise FetchRefusal(
                f"{self.connector_id}: sem license_id declarado em connectors.json — "
                "licença nunca é deduzida de página"
            )
    ...
    def _to_result(
        self, response: HttpResponse, decision: str, retries: int, waited: float
    ) -> FetchResult:
        return FetchResult(
            url=response.url,
