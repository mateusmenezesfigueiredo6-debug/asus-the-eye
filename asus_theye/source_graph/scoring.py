...
def methodology_version() -> str:
    return json.loads((DATA_DIR / "ranking_weights.json").read_text(encoding="utf-8"))[
        "methodology_version"
    ]
...
def score_source(
    ...
    *,
    ...
    limitations = [
        f"componente omitido por ausência de dado: {component_id} "
        f"(peso {weights[component_id]}) — omitido, não zerado"
    ...
    *,
    ...
    if entity_type == "person":
        raise ScoringError(
            ...
            "base legal mapeada e aprovação de DPO/jurídico (RELEASE_PROTOCOL.md). "
