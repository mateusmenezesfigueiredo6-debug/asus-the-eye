...
TAXONOMY_PATH = Path(__file__).resolve().parents[2] / "data" / "legal-taxonomy" / "legal_areas.master.json"
...
def coverage_by_track(report: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for entry in report["by_category"]:
        ...
        bucket = tracks.setdefault(
            track, {"categories": 0, "qualified": 0, "target": 0, "blocking_reasons": {}}
        )
        bucket["categories"] += 1
    ...
    for bucket in tracks.values():
        bucket["coverage_pct"] = (
            round(100 * bucket["qualified"] / bucket["target"], 2) if bucket["target"] else 0.0
        )
