...
def benchmark_page(report_path: str | Path = "reports/benchmark/latest.json") -> str:
    ...
    best_score = (
        f"{scores[best_score_solver]:.3f} ({best_score_solver})" if best_score_solver else "n/a"
    )
    best_time = (
        f"{times[best_time_solver]:.3f} ms ({best_time_solver})" if best_time_solver else "n/a"
    )
...
def register_benchmark_routes(
    app: Any, report_path: str | Path = "reports/benchmark/latest.json"
) -> None:
