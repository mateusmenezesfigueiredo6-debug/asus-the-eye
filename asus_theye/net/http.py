...
class Transport(Protocol):
    ...
    def request(
        self, url: str, *, headers: Mapping[str, str], timeout: int, max_bytes: int
    ) -> HttpResponse: ...
            ...
            with urllib.request.urlopen(request, timeout=timeout) as response:
                ...
                if len(body) > max_bytes:
                    raise ResponseTooLarge(
                        f"corpo excedeu {max_bytes} bytes: {url}", status=response.status
                    )
                return HttpResponse(
                    url=url,
...
def get_bytes(
    ...
    *,
    ...
    return (transport or UrllibTransport()).request(
