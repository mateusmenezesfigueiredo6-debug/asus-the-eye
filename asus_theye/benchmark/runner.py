...
def _append_history(report: dict[str, Any], path: Path) -> None:
    ...
    summary = {
        ...
        "times_ms": {
            name: result["execution_time_ms"] for name, result in report["results"].items()
        },
...
def run_benchmark_suite(
    *,
    ...
    qaoa = run_qaoa_benchmark(
        dataset, shots=shots, layers=layers, seed=seed, backend="local_simulator", ledger=ledger
    )
