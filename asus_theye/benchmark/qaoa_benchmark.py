...
def run_qaoa_benchmark(
    ...
    *,
    ...
    states = [
        tuple((state >> qubit) & 1 for qubit in range(normalized.variable_count))
    ...
    for bits in states:
        weight = sum(
            item_weight * bit for item_weight, bit in zip(normalized.weights, bits, strict=True)
        )
    ...
    for gamma in angle_grid:
        for beta in angle_grid:
            ...
            expected = sum(
                abs(amplitude) ** 2 * objective
                for amplitude, objective in zip(candidate, objectives, strict=True)
            )
    ...
    feasible_samples = [
        states[state] for state in sampled_states if normalized.is_feasible(states[state])
    ]
    solution = max(
