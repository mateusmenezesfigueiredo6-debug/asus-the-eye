...
LOST_REASONS = (
    "preco", "prazo", "escopo", "concorrencia", "sem_resposta",
    "desistiu", "fora_de_escopo", "outro",
)
SOURCE_CHANNELS = (
    "indicacao", "site", "evento", "prospeccao_ativa", "recorrencia", "parceria", "outro",
)
        ...
        *,
        ...
        if to_stage not in ALLOWED_TRANSITIONS[from_stage]:
            ...
            raise OpportunityError(
                f"transição inválida {from_stage!r} → {to_stage!r}; permitidas: {allowed}"
            )
        if to_stage == "perdido":
            if lost_reason not in LOST_REASONS:
                raise OpportunityError(
                    "perder exige motivo registrado — perder sem motivo não ensina nada"
                )
        if to_stage == "ganho":
