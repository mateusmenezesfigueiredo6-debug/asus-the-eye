...
"""Testes de sinais_juros — offline, com transporte injetado."""

from __future__ import annotations

import pytest

from asus_theye.markets.sinais_juros import (
    PESO_FOCUS,
    PESO_SELIC,
    SinaisJurosError,
    mediana_focus_selic,
    probabilidade_para_juros,
    sinais_para_juros,
)
from asus_theye.net.http import HttpError, HttpResponse

OLINDA_COM_MEDIANA = b'{"value":[{"Data":"2026-08-14","Mediana":13.75}]}'
OLINDA_VAZIO = b'{"value":[]}'
SGS_COM_META = b'[{"data":"30/09/2026","valor":"13,75"}]'
...
def test_mediana_focus_acha_a_linha() -> None:
    ...
    assert mediana_focus_selic("2026-09", transport=t) == (13.75, "2026-08-14")


def test_mediana_focus_mes_sem_linha_e_none() -> None:
    t = TransporteRoteado((200, OLINDA_VAZIO), (200, SGS_VAZIO))
    assert mediana_focus_selic("2026-09", transport=t) is None
...
def test_mediana_focus_malformado_levanta(status: int, body: bytes) -> None:
    ...
    with pytest.raises(SinaisJurosError):
        mediana_focus_selic("2026-09", transport=t)


def test_mes_referencia_invalido_levanta() -> None:
    with pytest.raises(SinaisJurosError, match="aaaa-mm"):
        mediana_focus_selic("09/2026", transport=TransporteRoteado((200, OLINDA_VAZIO), (200, SGS_VAZIO)))
...
def test_fonte_inalcancavel_levanta() -> None:
    ...
    with pytest.raises(SinaisJurosError, match="inalcançável"):
        mediana_focus_selic("2026-09", transport=TFalso())  # type: ignore[arg-type]
...
def test_sinais_direcao_e_peso_declarados() -> None:
    # Focus 13.75 >= 13.50 → SIM peso 20; SGS 13.75 >= 13.50 → SIM peso 10
    t = TransporteRoteado((200, OLINDA_COM_MEDIANA), (200, SGS_COM_META))
    sinais = sinais_para_juros("2026-09", 13.50, transport=t)
    assert [(s.direcao, s.peso) for s in sinais] == [("sim", PESO_FOCUS), ("sim", PESO_SELIC)]
    assert all(s.fonte for s in sinais)
    assert "Focus" in sinais[0].fonte and "432" in sinais[1].fonte


def test_sinais_direcao_nao_quando_abaixo_do_limiar() -> None:
    # Focus 13.75 < 14.00 → NÃO; SGS sem mês → ausente
    t = TransporteRoteado((200, OLINDA_COM_MEDIANA), (200, SGS_VAZIO))
    sinais = sinais_para_juros("2026-09", 14.00, transport=t)
    assert len(sinais) == 1
...
def test_sem_nenhuma_fonte_probabilidade_e_o_prior_honesto() -> None:
    t = TransporteRoteado((200, OLINDA_VAZIO), (200, SGS_VAZIO))
    prob = probabilidade_para_juros("2026-09", 13.50, transport=t)
