...
def gazette(trecho: str, municipio: str = "Planalto", uf: str = "SP",
            data: str = "2026-07-31", url: str = "https://exemplo/a.pdf") -> dict:
    return {"excerpts": [trecho], "territory_name": municipio, "state_code": uf,
            "date": data, "url": url}
...
def test_exigencia_de_certidao_nao_e_lead():
    ...
    ruido = ("b) Certidao negativa de Recuperacao Judicial ou Extrajudicial "
...
def test_qualificacao_economico_financeira_e_descartada():
    ruido = ("Documentos relativos a Qualificacao Economico-Financeira "
             "(Certidao Negativa de Falencia, Concordata, Recuperacao Judicial).")
    assert extrair(RECUPERACAO, [gazette(ruido)]) == []


def test_empresa_nomeada_e_lead():
    real = ("AMAZONIA PESCADOS LTDA EM RECUPERACAO JUDICIAL, inscrita com CNPJ "
...
def test_excerto_com_cpf_nao_entra_na_edicao():
    com_cpf = ("FULANO COMERCIO LTDA EM RECUPERACAO JUDICIAL, socio inscrito "
               "no CPF 123.456.789-00, comunica aos credores.")
...
def test_um_lead_por_publicacao():
    dois = ("ALFA COMERCIO LTDA EM RECUPERACAO JUDICIAL e tambem BETA "
...
def test_edicao_respeita_o_teto_de_leads():
    ...
    nomes = [f"EMPRESA {chr(65 + i)}{chr(65 + i)}{chr(65 + i)}{chr(65 + i)} COMERCIO"
             for i in range(MAX_LEADS + 8)]
    gazettes = [gazette(f"{nome} LTDA EM RECUPERACAO JUDICIAL informa.",
                        url=f"https://exemplo/{i}.pdf")
...
def test_render_escapa_html_do_diario():
    veneno = ("SELLIX AMBIENTAL LTDA EM RECUPERACAO JUDICIAL "
              "<script>alert(1)</script> comunica aos credores.")
