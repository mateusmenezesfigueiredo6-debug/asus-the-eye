...
def test_remove_so_a_linha_alvo(store: Path) -> None:
    r = redigir(store=store, campo_id="observacao_id", valor_id="aaa", motivo=MOTIVO_TERCEIRO, justificativa="termos")
...
def test_motivo_nao_declarado_levanta(store: Path) -> None:
    with pytest.raises(RedacaoError, match="motivo"):
        redigir(store=store, campo_id="observacao_id", valor_id="aaa", motivo="sei_la", justificativa="x")
...
def test_alvo_inexistente_levanta(store: Path) -> None:
    ...
    with pytest.raises(RedacaoError, match="nada a expurgar"):
        redigir(store=store, campo_id="observacao_id", valor_id="zzz", motivo=MOTIVO_TERCEIRO, justificativa="x")
...
def test_expurgo_sela_recibo_e_NAO_quebra_a_corrente(store: Path, tmp_path: Path) -> None:
    ...
    r = redigir(
        ...
        justificativa="termos de terceiro restringem armazenar e exibir",
        sdk=sdk,
        eventos=corrente,
    )
    ...
    assert len(depois) == len(antes) + 1  # o expurgo ADICIONA, nunca subtrai
