...
| `atlas-benchmark` | Mede as baselines do Atlas de Algoritmos (catálogo com hub/licença/versão, statsforecast) nos MESMOS folds walk-forward do nowcast, com a regra dura do baseline Focus, e registra corrida selada por algoritmo no mlops. | `asus-theye atlas-benchmark --json` |
| `markets-eleitoral-preview` | Roda o Modelo2026 (tempo de TV versionado + atenção Wikipédia ao vivo, com trava de identidade por artigo) e grava o retrato datado em `dados/modelo_2026_preview.json`, selado como `project.measurement`. Ordem, **nunca** previsão calibrada; família de atenção é tudo-ou-nada, exclusão sai com motivo. Roda 1×/dia por cron local. | `asus-theye markets-eleitoral-preview --json` |
...
| `markets-rodada` | **Laço diário**: consulta o gerador de cada mercado ABERTO, reprecifica o que mudou e grava ponto na série **sempre** — dia sem movimento também é informação para a calibração por horizonte. Cada claim é independente: falha em um não aborta os demais. | `asus-theye markets-rodada --json` |
| — | A rodada passa a gravar **duas trilhas** por mercado: a do Focus (que move `probability`) e a **própria**, vinda da cobertura noticiosa do GDELT, registrada em paralelo e selada antes do desfecho. Na liquidação as duas são pontuadas com a mesma régua. | ver `markets-rodada` |
